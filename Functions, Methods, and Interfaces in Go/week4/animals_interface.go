...
func main() {
	...
	for {
		...
		switch command {
		case "newanimal":
			NewAnimal(arg_1, arg_2, animal_map)
		case "query":
			Query(arg_1, arg_2, animal_map)
		default:
...
func NewAnimal(a_name string, a_type string, animal_map map[string]Animal) {
	//new_animal := new(Animal)
	switch a_type {
	case "cow":
		animal_map[a_name] = *new(Cow)
	case "bird":
		animal_map[a_name] = *new(Bird)
	case "snake":
		animal_map[a_name] = *new(Snake)
	default:
...
func Query(a_name string, info string, animal_map map[string]Animal) {
	if animal_obj, ok := animal_map[a_name]; ok {
		GetInfo(animal_obj, info)
	} else {
