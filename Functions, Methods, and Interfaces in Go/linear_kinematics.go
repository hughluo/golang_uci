 ...
 * The function returned by GenDisplaceFn() should take one float64 argument t, representing time, and return one float64 argument 
 * which is the displacement travelled after time t.
...
func main() {
	...
	var vo float64
	var so float64
	fmt.Println("Type in value for acceleration a...")
	fmt.Scan(&a)
	fmt.Println("Type in value for initial velovity vo...")
	fmt.Scan(&vo)
	fmt.Println("Type in value for initial displacement so...")
	fmt.Scan(&so)

	var t float64
	fmt.Println("Type in value for time t...")
	fmt.Scan(&t)

	fn := GenDisplaceFn(a, vo, so)
	fmt.Println("Displacement is ...")
	fmt.Println(fn(t))
}
